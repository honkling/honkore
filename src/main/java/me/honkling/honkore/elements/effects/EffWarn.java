...
public class EffWarn extends Effect {

	static {
		Skript.registerEffect(EffWarn.class, "[honkore] make %commandsender% warn %players% [(for|because|with reason|due to) %-strings%]");
	}
	...
	public String toString(@Nullable Event event, boolean debug) {
		return "honkore warn effect with moderator " + moderator.toString(event, debug) + "warning players " + player.toString(event, debug) + " and reason " + reason.toString(event, debug);
	}
