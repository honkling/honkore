...
public class EffMute extends Effect {
	...
	static {
		Skript.registerEffect(EffMute.class, "[honkore] make %commandsender% mute %players% [(for|because|with reason|due to) %-strings%] [(for|with) [length] <^(([0-9]{1,}(s|m|h|d|w|mo|y|de)){1,}|forever)$>]");
	}
	...
	public String toString(@Nullable Event event, boolean debug) {
		return "honkore mute effect with moderator " + moderator.toString(event, debug) + "muting players " + player.toString(event, debug) + " and reason " + reason.toString(event, debug) + " and length " + length;
	}
